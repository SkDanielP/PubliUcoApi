...
public final class identificadorValidation implements Validation<UUID> {
	...
	public final Result execute(UUID data) {
		...
		if(UtilObject.isNull(data)) {
			result.addMessage("No es posible continuar con el identificador del estado administrador categoria no puede ser vacio");
		}
		
		else if(UtilUUID.isDefault(data)) {
			result.addMessage("No es posible tener el identificador por defecto del estado administrador categoria");
		}
