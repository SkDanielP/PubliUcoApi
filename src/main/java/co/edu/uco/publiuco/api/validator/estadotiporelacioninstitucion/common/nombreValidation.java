...
public final class nombreValidation implements Validation<String>{
	...
	public final Result execute(String data) {
		...
		if(UtilText.getUtilText().isEmpty(data)) {
			...
			if(true)
				result.addMessage("El nombre del estado tipo relacion institucion solo debe tener letras y espacios");
			
			if(true) { //validar longitud de la cadena entre 1 y 30
			result.addMessage("El nombre del estado tipo relacion institucion debe tener una longitud entre 1 y 30");
		}
		}
		return result;
	}
